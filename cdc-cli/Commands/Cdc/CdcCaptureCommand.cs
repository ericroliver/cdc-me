...
/// <summary>
/// Command to capture CDC data without stopping CDC operations
/// </summary>
public class CdcCaptureCommand : ApiCommandBase
{
    /// <summary>
    /// Initializes a new instance of the CdcCaptureCommand class
    /// </summary>
    ...
    public CdcCaptureCommand(
        ICdcApiClient apiClient,
        IJsonHandler jsonHandler,
        ILogger<CdcCaptureCommand> logger,
        CliConfiguration configuration)
        : base("capture", "Capture CDC data without stopping CDC (CDC remains active)", apiClient, jsonHandler, logger, configuration)
    {
        ConfigureCommand();
    }
    ...
    private void ConfigureCommand()
    {
        ...
        var typeOption = new Option<string>(
            aliases: new[] { "--type", "-t" },
            description: "Capture type (Intermediate, Checkpoint, etc.)",
            getDefaultValue: () => "Intermediate");
    ...
    /// <summary>
    /// Executes the capture CDC command
    /// </summary>
        ...
        string? file)
    {
        try
        {
            ...
            // Execute API call
            var response = await ExecuteApiCallAsync<CaptureCdcRequest, CaptureCdcResponse>("/api/cdc/capture", request);
            if (response == null)
            {
                return; // Error already handled
            }
    ...
    private async Task<CaptureCdcRequest?> BuildRequestAsync(
        string? session,
        string? capture,
        string type,
        string? data,
        string? file)
    {
        ...
        {
            return await GetRequestAsync<CaptureCdcRequest>(data, null, false);
        }

        // Priority 2: --file (JSON from file)
        if (!string.IsNullOrWhiteSpace(file))
        {
            return await GetRequestAsync<CaptureCdcRequest>(null, file, false);
        }
        ...
        if (string.IsNullOrWhiteSpace(session) && string.IsNullOrWhiteSpace(capture))
        {
            ...
            return await GetRequestAsync<CaptureCdcRequest>(null, null, true);
        }

        // Priority 4: CLI parameters
        return new CaptureCdcRequest
        {
