...
namespace Softbase.Cdc.Trace
{
    public class TraceManager : ITraceManager
    {
        ...
        public async Task<TraceSession> StartTraceAsync(TraceConfiguration config, string testConnectionString)
        {
            ...
            try
            {
                ...
                session.TestConnectionString = testConnectionString;
        ...
        public async Task<TraceSession> StopTraceAsync(Guid sessionId)
        {
            ...
            try
            {
                ...
                // Export trace data from Extended Events to trace database
                await ExportTraceDataAsync(sessionId, sessionName);
                ...
                var events = await _testDac.ExecuteReaderAsync(readEventsSql, reader =>
                {
                    ...
                    while (reader.Read())
                    {
                        var traceEvent = new TraceEvent
                        {
                            ...
                            ExecutionOrder = reader.GetInt64(12),
        ...
        public async Task<bool> IsTraceRunningAsync(string sessionName)
        {
            try
            {
                ...
                var extendedEventsSessionName = $"CDC_Trace_{session.SessionId:N}";
                var checkSql = @"
                    SELECT COUNT(1)
                    FROM sys.dm_xe_sessions
                    WHERE name = @sessionName";
