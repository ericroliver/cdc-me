...
CREATE INDEX IF NOT EXISTS idx_cdc_capture_headers_name 
    ON cdc_capture_headers(capture_name);
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE cdc_captures 
            ADD COLUMN capture_header_id UUID REFERENCES cdc_capture_headers(capture_header_id) ON DELETE CASCADE;
...
SELECT 'cdc_capture_headers table created successfully!' as result;